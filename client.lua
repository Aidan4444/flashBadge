...
AddEventHandler('flashBadge:client:animation', function()
    Citizen.CreateThread(function()
        ...
        AttachEntityToEntity(prop, ped, boneIndex, 0.065, 0.029, -0.035, 80.0, -1.90, 75.0, true, true false, true, 1, true)
        RequestAnimDict('paper_1_rcm_alt1-9')
        TaskPlayAnim(ped, 'paper_1_rcm_alt_1-9', 'player_one_dual-9', 8.0, -8, 10.0, 49, 0, 0, 0, 0)
        Citizen.Wait(3000)
        ...
        if config.useCommand then 
            if leoAcePerm then
                ...
                TriggerEvent('chat:addMessage', {
                    color = {255, 92, 92},
                    multiline = true,
                    args = {'[Error]', cofig.commandErrorMessage}
                })
            end
        end
    end 
